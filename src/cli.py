...
Exemplos de uso:
  ...
  %(prog)s info --type degree           # Mostra ranking por grau
        ...
        path_parser = subparsers.add_parser(
            "path", help="Encontra caminho entre dois bairros usando Dijkstra"
        )
        path_parser.add_argument("start", help="Bairro de origem")
        path_parser.add_argument("end", help="Bairro de destino")

        # Comando: process
        process_parser = subparsers.add_parser(
            "process", help="Processa dados de entrada -> bairros_unique.csv"
        )
        ...
        info_parser = subparsers.add_parser(
            "info", help="Exibe informações sobre os resultados das análises"
        )
    ...
    def cmd_path(self, args) -> int:
        try:
            ...
            if args.start == "Nova Descoberta" and args.end == "Boa Viagem (Setúbal)":
                ...
                Path(PERCURSO_NOVA_DESCOBERTA_SETUBAL_PATH).parent.mkdir(
                    parents=True, exist_ok=True
                )
                with open(
                    PERCURSO_NOVA_DESCOBERTA_SETUBAL_PATH, "w", encoding="utf-8"
                ) as f:
    ...
    def cmd_interactive(self, args) -> int:
        try:
            ...
            output_path = visualizer.viz_interactive_graph(
                graph=graph, caminho_destacado=caminho_destaque
            )
    ...
    def cmd_plots(self, args) -> int:
        try:
            ...
            print(f"✓ Arquivos salvos no diretório: {PART1_Q8_DIR}")
            print("=" * 60)
            return 0
        ...
        try:
            ...
            if args.type == "global":
                ...
                for i, item in enumerate(data[:10], 1):
                    ...
                    print(
                        f"     Grau: {item['grau']}, Densidade ego: {float(item['densidade_ego']):.4f}"
                    )
